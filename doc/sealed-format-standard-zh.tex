...
隐私计算与数据交付场景中，密文容器除机密性、完整性外，还须支持\textbf{随机访问}：在不解封整包的前提下，按块定位并读取、校验其中一段内容，以满足审计、抽样与\textbf{数据分析}等需求。若只能整包顺序解密，上述场景成本过高；若随机访问依赖与密文格式无关的外部索引，则难以在保持分段认证的前提下做范围读取，\textbf{数据分析}链路也难以稳定复用同一份 AEFF 产物。

\textbf{与常见密文形态对比。}业界与隐私计算链路中广泛使用的做法与本标准目标差异可概括如下（均为\textbf{已有}常见形态，非本标准所定义）：
\begin{itemize}
  \item \textbf{整文件单一密文}（如 OpenSSL \texttt{enc}、国密算法对明文一次性对称加密等）：实现简单，但密文流\textbf{不含}与内容对齐的块级索引；\textbf{随机访问}往往只能做未与格式绑定的字节范围读取，或须从头解密，无法经济地完成分段认证与\textbf{数据分析}所需的局部校验。
  ...
  \item \textbf{单管线消息型密文}（如 OpenPGP 对称加密的单文件输出）：面向顺序传输与整消息认证，\textbf{通常不提供}与内容区块索引等价的内建\textbf{随机访问}语义；多文件语义须由外层协议另行约定。
  \item \textbf{仅顺序解封的密封实现}（在加密之上未标准化块索引、尾部元数据与目录树时）：与整文件密文类似，外部系统若自建偏移表，易出现索引与密封载体脱节、版本不一致等问题。
\end{itemize}
...
\begin{itemize}
  ...
  \item \ttf{start_file_pos}、\ttf{end_file_pos}：本 Block 在 \texttt{.aeff} 文件 Content Region 内占据的字节半开区间 \ttf{[start, end)}，偏移以内容区首字节为 \texttt{0}；\textbf{不}表示业务侧源文件内偏移（见第~\ref{sec:block-meta-fields} 节）。
\end{itemize}
