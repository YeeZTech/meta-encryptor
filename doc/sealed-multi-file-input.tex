% 附录 G：多文件封装与解封（参考）
% 附录 G：在 Item 管道（附录 H）之上编排多逻辑文件与目录树；目录明文格式见第 7 章。
...
\begin{enumerate}
  ...
  \item \textbf{收尾落盘}（第~\ref{sec:seal-finalize} 节）：回写 Header 至文件首部并写出 Block Infos 区（\ttf{tree_item_index}、\ttf{tree_item_number} 取自封装上下文）。
\end{enumerate}
