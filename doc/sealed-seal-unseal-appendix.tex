...
\begin{enumerate}
  ...
  \item 按第~\ref{sec:item-record} 节向内容区（自文件偏移 \ttf{content_offset} 起）追加 \texttt{UINT64(LE) item\_size} 与 \ttf{cipher_payload}。
\end{enumerate}
...
\begin{itemize}
  \item 含义：本 Block 在 \texttt{.aeff} 文件 \textbf{Content Region} 内占据的字节半开区间 \texttt{[start\_file\_pos, end\_file\_pos)}。偏移以内容区首字节为 \texttt{0}（内容区在文件内起始于 \ttf{content_offset} $=$ \ttf{header_size}，见第~\ref{sec:region-length} 节）。\textbf{不包含} Header 区与 Block Infos 区。
  ...
  \item \textbf{不是}：被封装前原始明文文件（或任意宿主路径下文件）内的字节偏移。字段名 \texttt{file} 指本 \texttt{.aeff} 输出文件的内容区（与参考实现 \texttt{block\_info\_t.start\_file\_pos} 一致），勿与「源文件偏移」混读；内容区内偏移 \texttt{0} 对应文件物理偏移 \ttf{content_offset}。
\end{itemize}
...
内容区全部 Item 写出后，按物理布局 \texttt{[Header][Content Region][Block Infos]} 完成收尾：回写 Header 至文件首部，并将 Block Infos 区写入文件末尾。
...
\textbf{相关格式：}
三区长度与偏移见第~\ref{sec:region-length} 节；Block Info、Header 布局见第~\ref{sec:block-infos}、\ref{sec:header-region} 节。
最终物理布局 MUST 为：Header（偏移 \texttt{0}）$\rightarrow$ Content Region（偏移 \ttf{content_offset}）$\rightarrow$ Block Infos（偏移 \ttf{block_infos_offset}）（第~\ref{sec:seal-consistency} 节）。

\textbf{实施步骤：}
\begin{enumerate}
  \item \textbf{写出 Block Infos 区：}记 \ttf{N = block_number}，自偏移 \ttf{block_infos_offset}（$=$ \ttf{header_size} $+$ \ttf{content_size}）起顺序写入 \ttf{N} 条 Block Info（各 32 字节，取值见第~\ref{sec:block-meta} 节）；\ttf{block_infos_size} $= 32 \times N$；
  \item \textbf{写出 Header 区：}在偏移 \texttt{0} 处按第~\ref{sec:header-region} 节写入 80 字节 Header；\ttf{magic_number}、\ttf{version_number}、\ttf{tree_item_index}、\ttf{tree_item_number} 取自封装上下文；\ttf{item_number}、\ttf{block_number} 取自 Item 管道；Header.\ttf{data_hash} MUST 取 $H_n$（\textbf{不得}以 Item 密文另行摘要替代）。
\end{enumerate}
完成后 \ttf{file_size} $=$ \ttf{header_size} $+$ \ttf{content_size} $+$ \ttf{block_infos_size}。

\textbf{说明：}
Item 管道主循环 MAY 自偏移 \ttf{content_offset} 起顺序写出 Content Region；实现 MAY 预留首部 80 字节、使用临时缓冲或在收尾阶段组装，只要最终布局与本节一致。
...
\begin{itemize}
  \item 最终物理布局 MUST 为：Header（偏移 \texttt{0}）$\rightarrow$ Content Region（偏移 \ttf{content_offset}）$\rightarrow$ Block Infos（偏移 \ttf{block_infos_offset}）；
  \item Content Region MUST 起始于偏移 \ttf{header_size}，Block Infos 区 MUST 起始于偏移 \ttf{header_size} $+$ \ttf{content_size}；
...
从 \texttt{.aeff} 文件首部（偏移 \texttt{0}）取得 Header 记录，校验识别字段并计算三区长度与偏移（见第~\ref{sec:read-model} 节）。
...
\begin{itemize}
  ...
  \item \ttf{content_size}、\ttf{block_infos_size}、\ttf{content_offset}、\ttf{block_infos_offset}（由 \ttf{file_size} 与 \ttf{block_number} 按第~\ref{sec:region-length} 节算出）。
\end{itemize}
...
\textbf{实施步骤：}
\begin{enumerate}
  \item 读取文件首部 80 字节（偏移 \texttt{0}），按第~\ref{sec:header-region} 节解析，得到各 Header 字段；
  ...
  \item 令 \ttf{N} $=$ \ttf{block_number}，按第~\ref{sec:region-length} 节计算 \ttf{content_offset}、\ttf{block_infos_offset}、\ttf{block_infos_size} 与 \ttf{content_size}；若 \ttf{content_size} 不大于 \texttt{0}，MUST 终止解封，错误码 SHOULD 为附录 C 中的 \ttf{E_FORMAT_TOO_SMALL}（或等价格式错误码）。
\end{enumerate}
...
\textbf{输入：}
\begin{itemize}
  \item \ttf{content_offset}、\ttf{content_size}、\ttf{item_number}（由第~\ref{sec:unseal-header-model} 节得到）。
\end{itemize}
