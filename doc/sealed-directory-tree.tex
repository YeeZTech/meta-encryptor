...
自本版起，通过\textbf{目录树文件}在单条 \texttt{.sealed} 内索引多个逻辑文件与子目录：封装时将宿主侧目录树（各文件的\textbf{相对路径}与层级）写入目录明文，解封后在集成层指定的输出根路径下按相同相对路径还原，以保留与密封前一致的目录结构（第~\ref{sec:multi-file-chapter} 章给出封装与整包解封流程）。
...
\begin{itemize}
  ...
  \item 本章规定目录明文的\textbf{二进制布局、解析与校验}；封装侧如何\textbf{生成}该明文、Header 目录树 Item 定位字段如何取值及解封侧如何\textbf{消费}解析结果，见第~\ref{sec:multi-file-chapter} 章。
  \item 本章\textbf{不}重复第~\ref{sec:unseal-chapter} 章的 Item 认证解密步骤；解封侧须先按第~\ref{sec:multi-file-tree-payload-concat} 节得到目录明文字节串，再调用本章解析。
\end{itemize}
...
\begin{itemize}
  ...
  \item 目录树明文格式版本由树头.\ttf{version} 表示，与 \texttt{.sealed} 文件 Header.\ttf{version_number} \textbf{相互独立}；当前版本约定值见第~\ref{sec:tree-header} 节（暂定）。
\end{itemize}
...
树头.\ttf{version} 仅描述目录明文布局演进，与 \texttt{.sealed} Header.\ttf{version_number} \textbf{无}对应关系，二者 MAY 独立递增；读取器 MUST 在解析目录节前校验 \ttf{magic_number} 与 \ttf{version}；若后续调整树头布局或上述常量，树头.\ttf{version} MUST 递增并提供兼容说明，\textbf{不必}与 sealed 文件版本同步。
